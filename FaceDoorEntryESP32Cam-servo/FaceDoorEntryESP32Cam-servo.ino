...
#include <ESP32Servo.h> // Incluir la biblioteca del Servo Motor

const char* ssid = "proa";
const char* password = "proa.2024";
...
#define servo_pin 2 // pin donde está conectado el servo
...
void setup() {
  ...
  myservo.attach(servo_pin); // Iniciar el servo motor
  myservo.write(0); // Posicionar el servo en el ángulo inicial (0 grados)
...
void open_door(WebsocketsClient &client) {
  myservo.write(90); // Girar el servo 90 grados
  Serial.println("Door Unlocked");
  client.send("door_open");
  door_opened_millis = millis(); // time servo turned and door opened
}
...
void loop() {
  ...
  while (client.available()) {
    ...
    if (millis() - interval > door_opened_millis && myservo.read() == 90) { // current time - face recognised time > 5 secs
      myservo.write(0); // Retornar el servo a la posición inicial
